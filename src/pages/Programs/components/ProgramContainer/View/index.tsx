...
const ProgramView: React.FC<ProgramViewProps> = ({ setIsUpdate, onDismiss: onFatherDismiss }) => {
    ...
    <Container onMouseDown={e => e.stopPropagation()} layoutId={selected?.toString()}>
      ...
      <div style={{ margin: 20 }}>
        <ImageOverlay>
          ...
          <div>
            <ImageDelete title={confirmDelete ? 'Clique para confirmar' : 'Excluir programa'} confirm={confirmDelete} onClick={confirmDelete ? () => handleDelete() : () => setConfirmDelete(true)}>
            ...
            <ImageEdit title="Alterar programa" onClick={() => setIsUpdate(true)}>
